...
import ec.europa.eu.testcentre.client.gui.ServletDeployer.GuiCommande;
...
public class SinglePanelCounter extends JPanel implements ActionListener {
	...
	public SinglePanelCounter(JFrame frame ,final String host,final String port){
		init();
		this.host=host;
		this.port=port;
		this.frame= frame;
		
		GuiCommande comm = new GuiCommande( frame) {
	...
	private void init() {
		...
		domainComboBox =new JComboBox() {
		    public void addItem(Object anObject) {		    	
		    	...
		    	ActionListener[] old= getActionListeners();
			    for (int i = 0; i < old.length; i++) {
			    	removeActionListener(old[i]);
		...
		root = new DefaultMutableTreeNode("avavilable point of monitoring");
	...
	private void doChooseType() {
		oldtype = newtype;
		newtype = typecomboBox.getSelectedItem();
		setName((String)newtype);	
int index = ((JTabbedPane)getParent()).getSelectedIndex();
((JTabbedPane)getParent()).setTitleAt(index, (String)newtype);
		if( !( newtype == null || newtype.equals(oldtype) )){
			root.removeAllChildren();
			attributTree.validate();
			attributTree.doLayout();
			attributTree.collapseRow(0);
	...
	private void doChooseDomain() {
		olddomain=newdomain;
		newdomain = domainComboBox.getSelectedItem();
	...
	private void populateAttribut(final Object newtype) {	
		
		GuiCommande comm = new GuiCommande(frame) {
			JmxConnection connection;
			cec.monitor.type.Counter[] count;
			@Override
			protected void front() throws Exception {
				...
				for (int i = 0; i < count.length; i++) {				
					 try {
						...
						attributTree.setModel(new DefaultTreeModel(root));
					/*	attributTree.validate();
						attributTree.doLayout();*/
						attributTree.collapseRow(0);
			...
			protected void back() throws Exception {
				 connection = new JmxConnection(host, port);
				 count= connection.getMBeanByType((String)newtype, (String) domainComboBox.getSelectedItem());
			}
		};
	...
	private void populateType(final Object newdomain) {
		...
		GuiCommande comm = new GuiCommande(frame) {
			...
			protected void back() throws Exception {
				connection = new JmxConnection(host, port);
				 ret = connection.getAllTypesForDomain((String)newdomain);
			}
		};
	...
	private static class TypeNodeData {
		...
		public  String toString() {
			...
			try {
				...
				ret =JmxConnection.getPrettyName(objname);
